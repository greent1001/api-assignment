import 'dart:convert';
import 'dart:io';

import 'package:projectapi/model/topnews_model.dart';
import 'package:projectapi/util/failure.dart';
import 'package:http/http.dart' as http;

class ApiCallClass {

  Future<List<Article>> getNews() async {
    try {
      final news = await http.get(
        Uri.parse(
            "https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=645caab6db714a01858f10d81b219a6b"),
      );
      if (news.statusCode == 200) {
        final Iterable rawJson = jsonDecode(news.body)["articles"];
        return rawJson.map((article) => Article.fromJson(article)).toList();
      } else {
        throw Failure(message: news.body.toString());
      }
    } on SocketException {
      throw Failure(message: "You are not connected to the Internet");
    } catch (error) {
      throw Failure(message: error.toString());
    }
  }
}
